...
export function Footer({ siteData }: { siteData: SiteContent | null }) {
    return (
        <footer className="bg-secondary text-secondary-foreground pt-16 pb-24 md:pb-8">
            <div className="container mx-auto px-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
                    ...
                    <div className="space-y-4">
                        ...
                        <p className="text-muted-foreground text-sm leading-relaxed max-w-xs">
                            {siteData?.hero_subtitle || "Where Dreams Meet Wagging Tails! We are a premium dog breeding and adoption service dedicated to connecting deserving dogs with loving families."}
                        </p>
                    ...
                    <div className="space-y-4">
                        ...
                        <ul className="space-y-3 text-sm text-muted-foreground">
                            <li className="flex items-start gap-3">
                                ...
                                <span>{siteData?.address || "23, 5th Cross, Narayanappa Block, Benson Town, Bengaluru, Karnataka 560046"}</span>
                            </li>
                            <li className="flex items-center gap-3">
                                <Phone className="w-5 h-5 text-primary shrink-0" />
                                <span>{siteData?.phone || "+91 9060602037"}</span>
                            </li>
                            <li className="flex items-center gap-3">
                                <Mail className="w-5 h-5 text-primary shrink-0" />
                                <span>{siteData?.email || "hello@dogsparaside.in"}</span>
                            </li>
                        </ul>
                    </div>
                ...
                <div className="border-t border-secondary-foreground/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-xs font-medium text-muted-foreground text-center md:text-left">
                    <p>{siteData?.footer_text || `© ${new Date().getFullYear()} DogsParadise. All rights reserved.`}</p>
