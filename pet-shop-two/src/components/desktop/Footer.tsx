...
export default function Footer({ siteData }: { siteData: SiteContent | null }) {
            ...
            <div className="max-w-7xl mx-auto px-6 pt-16 pb-8">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <span className="font-heading text-2xl font-bold">
                                {siteData ? (
                                    <>
                                        {siteData.site_name.split(' ')[0]}
                                        <span className="text-primary">
                                            {siteData.site_name.substring(siteData.site_name.indexOf(' '))}
                                        </span>
                                    </>
                                ) : (
                ...
                <div className="border-t border-white/10 pt-8 mb-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center md:text-left">
                        <div className="flex items-center gap-3 justify-center md:justify-start">
                            ...
                            <span className="text-white/60 text-sm">{siteData?.address || "123, Happy Street, Pet Town, India"}</span>
                        </div>
                        <div className="flex items-center gap-3 justify-center md:justify-start">
                            <span className="text-xl">📞</span>
                            <a href={`tel:${siteData?.phone ? siteData.phone.replace(/[^0-9]/g, "") : "+919876543210"}`} className="text-white/60 text-sm hover:text-primary transition-colors">
                                {siteData?.phone || "+91 98765 43210"}
                            </a>
                        </div>
                        <div className="flex items-center gap-3 justify-center md:justify-start">
                            <span className="text-xl">📧</span>
                            <a href={`mailto:${siteData?.email || "hello@happypets.in"}`} className="text-white/60 text-sm hover:text-primary transition-colors">
                                {siteData?.email || "hello@happypets.in"}
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div className="border-t border-white/10 pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
                    <p className="text-white/40 text-xs">
                        {siteData?.footer_text || `© ${currentYear} Happy Pets. All rights reserved. Made with ❤️ for pet lovers.`}
                    </p>
