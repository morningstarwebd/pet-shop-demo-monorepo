...
### ⑯ Studio — Dogs Paradise (Next.js 15 + Firebase + Genkit AI + shadcn/ui)
**Domain**: Premium pet breeding & adoption platform
**Stack**: Next.js 15 App Router, React 19, TypeScript, Tailwind CSS, Firebase, Google Genkit (Gemini 2.5 Flash), shadcn/ui, Framer Motion, Recharts
**Key modules**: `Navbar`, `Footer`, `MobileNav`, `WhatsAppFloating`, full shadcn/ui library (`accordion`, `carousel`, `chart`, `dialog`, `form`, `sheet`, `sidebar`, `table`, `tabs`, `toast`, `tooltip`, etc.)
**Routes**: `/` (home), `/pets` (breed catalog), `/shop`
**Features**:
- **Google Genkit AI** — Gemini 2.5 Flash integration via `src/ai/genkit.ts`; plug in any AI-powered feature
- **WhatsApp Floating Button** — `WhatsAppFloating.tsx`; auto-visible after 1.5s delay; opens wa.me with pre-filled message; green `#25D366` branded; mobile-safe z-index
- **Framer Motion animations** — `fadeIn`, `slideLeft`, `slideRight`, `staggerContainer` variants; `whileInView` with `once: true`
- **Firebase backend** — auth + data ready
- **Recharts** — data visualization components
- **React Hook Form + Zod** — form validation
- **Embla Carousel** — product/breed carousel
- **Full shadcn/ui** — 30+ production-ready UI components
**Design**: Clean modern light theme; primary teal; font-headline + font-accent; warm pet-friendly aesthetics
**Adaptable to**: Any service business needing AI features, WhatsApp integration, and a full component library

---

### ⑰ Constellation Linktree (Next.js 16 + Three.js + Tailwind v4)
**Domain**: Creative personal social links / linktree page with immersive 3D sky
**Stack**: Next.js 16, React 19, TypeScript, Tailwind CSS v4, Three.js 0.183
**Key modules**: `DynamicSkyOverlay`, `StarField`, `ConstellationGroup`, `ConstellationLines`, `IconBubble`, `LinkModal`
**Assets**: `galaxy-background.webp`, `person-silhouette.webp`
**Features**:
- **DynamicSkyOverlay** — Three.js WebGLRenderer fullscreen quad; GLSL fragment shader with `hash()`, `noise()`, `fbm()` (4-octave Fractal Brownian Motion); star detection via luminance threshold; sky mask + silhouette exclusion; realistic twinkling with slow-drift + fast-flicker layers
- **StarField** — canvas-based procedural star field with parallax effect
- **ConstellationGroup** — floating icon bubbles arranged in constellation pattern
- **ConstellationLines** — SVG/canvas lines connecting icon bubbles
- **IconBubble** — glassmorphism circle buttons; inline SVG icons (twitter, instagram, linkedin, github, youtube, tiktok + more); neon blue glow + backdrop-blur; hover scale + border glow animation; opens LinkModal
- **LinkModal** — zoom-in animated modal with link details
- **Tailwind CSS v4** — latest version with PostCSS config
- **Next.js 16** — latest version
**Design**: Deep space dark (`#020510`), neon blues `rgba(100,160,255)`, glassmorphism with `backdrop-filter: blur`, box-shadow neon glows
**Adaptable to**: Any creative portfolio, agency links page, personal brand page needing ambient animated background

---

### ⑱ MorningStar Web — Agency CMS (Next.js + Supabase + R3F + GSAP + MDX)
**Domain**: Web design agency portfolio with full headless CMS & admin dashboard
**Stack**: Next.js App Router, TypeScript, Tailwind CSS, Supabase (auth + DB), React Three Fiber + Drei + Postprocessing, Three.js, GSAP, Lenis, Framer Motion, MDX, Resend, Zustand, dnd-kit, Sentry, next-sitemap
**Key modules**:
- **CMS/Admin**: `LiveBlockEditor`, `SchemaFormBuilder`, `AdminSidebar`, `MediaPickerModal`, `ImageUpload`, `LinkPicker`, `PresenceBanner`, `GlobalSettingsCard`
- **Blocks**: `hero.block`, `about.block`, `blog.block`, `contact.block`, `featured-works.block`, `projects.block`, `marquee.block`, `header.block`, `footer.block`
- **3D/Visual**: `GalaxyScene`, `LiquidScene`, `StarScene`, `Particles`, `CameraRig`, `canvas-wrapper`, `hero-scene`
- **Visual Effects**: `aurora-effect`, `bokeh-effect`, `fog-effect`, `neural-network-effect`, `water-effect`, `wire-grid-effect`, `gradient-orb-effect`, `fluid-overlay`
- **Blog**: `blog-card`, `blog-list-client`, `mdx-components`, `reading-progress`, `share-buttons`, `table-of-contents`
- **Core**: `page-transition`, `app-loader`, `SmartImage`, `json-ld`, `ErrorBoundary`, `gsap-provider`, `lenis-provider`, `theme-provider`, `global-blocks`, `page-block-renderer`, `contact-form`
- **Projects**: `project-card`, `projects-client`
**Routes**: `/`, `/blog`, `/blog/[slug]`, `/projects`, `/projects/[slug]`, `/[slug]` (dynamic CMS pages), full `/admin/*` dashboard
**API Routes**: `/api/contact`, `/api/og` (OG image gen), `/api/admin/users`, `/api/cron/publish-scheduled`, `/api/keepalive`
**Stores**: `admin-data-store`, `editor-store`, `page-editor-store` (Zustand)
**Features**:
- **Live Block Editor** — drag-and-drop page builder with real-time preview; schema-driven form generation
- **Supabase Auth** — admin whitelist + role management + multi-user presence detection
- **MDX Blog** — reading progress bar, table of contents, share buttons, syntax highlighting (rehype-pretty-code), GFM
- **3D Scenes (R3F)** — GalaxyScene, LiquidScene, StarScene, Particles with postprocessing
- **GSAP + Lenis** — scroll animations + buttery smooth scrolling
- **Resend Email** — contact form with `ContactEmail.tsx` React email template
- **Sentry** — client + server error tracking; admin error dashboard with resolve button
- **Scheduled Publishing** — cron API route for auto-publishing scheduled posts
- **OG Image Generation** — dynamic OpenGraph images via API route
- **Auto sitemap + robots.txt** — next-sitemap integration
- **GitHub Actions CI** — automated lint + type-check pipeline
- **Lighthouse Auditing** — built-in performance audit scripts
- **JSON-LD** — structured data component
**Design**: Dark professional agency aesthetic; custom visual effects; next-themes dark/light
**Adaptable to**: Any agency, freelancer, or studio needing a full portfolio + CMS + blog + admin; the most advanced project in the library

---

### ⑲ Naaz Kool Pet Shop (Next.js 16 + Framer Motion v12 + Tailwind v4)
**Domain**: Premium cool-aesthetic pet shop / brand website
**Stack**: Next.js 16, React 19, TypeScript, Tailwind CSS v4, Framer Motion v12, Lucide React
**Key modules**: `HeroSection`, `FeaturedPets`, `BreedsCarousel`, `ProductShop`, `GroomingTeaser`, `BlogPreview`, `Testimonials`, `CommunityBanner`, `NaazKoolPromise`, `Navbar`, `Footer`, `MarqueeTicker`, `SideDrawer`, `GrainOverlay`
**Features**:
- **GrainOverlay** — CSS/canvas film grain texture overlay for premium tactile feel
- **MarqueeTicker** — infinite scrolling announcement/trust bar
- **SideDrawer** — slide-in mobile navigation drawer
- **BreedsCarousel** — horizontal scrollable breed showcase
- **GroomingTeaser** — grooming services preview section
- **CommunityBanner** — community engagement / social proof section
- **NaazKoolPromise** — brand trust + values section
- **BlogPreview** — latest blog posts preview
- **Framer Motion v12** — latest animations throughout all components
- **Tailwind CSS v4** — latest with PostCSS config
- **Mock data architecture** — `mockData.ts` + `animations.ts` utility
**Design**: Cool/premium pet aesthetic; grain texture overlay; Framer Motion v12 entrance animations; modern typography
**Adaptable to**: Any premium lifestyle brand, pet business, grooming service needing a cool modern look with grain texture aesthetic

---

## UPDATED COMPOSABILITY MATRIX

| Need → | UI Shell | E-commerce | Appointments | Map/Geo | AI Chat | Business Intel | 3D/Ambient | CMS/Admin | WhatsApp |
|--------|----------|------------|--------------|---------|---------|----------------|------------|-----------|----------|
| **Studio** | ✅ light modern + shadcn | ✅ shop page | ❌ | ❌ | ✅ Genkit/Gemini | ❌ | ❌ | ✅ Firebase | ✅ floating button |
| **Constellation Linktree** | ✅ deep space glassmorphism | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ Three.js GLSL stars | ❌ | ❌ |
| **MorningStar Web** | ✅ dark agency premium | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ R3F Galaxy/Liquid/Stars | ✅ Full CMS + blog + admin | ❌ |
| **Naaz Kool** | ✅ cool grain premium | ✅ product shop | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |

---

## NEW MIXING EXAMPLES

### Example D: Web Design Agency site (Morning + Constellation)
1. Use MorningStar's full CMS + admin as the base
2. Replace hero background with Constellation's `DynamicSkyOverlay` (Three.js GLSL stars)
3. Use Constellation's `IconBubble` + `ConstellationGroup` for floating service links
4. Add Studio's `WhatsAppFloating` for instant client contact
5. Result: Stunning agency site with 3D star hero + full CMS + WhatsApp lead capture

### Example E: Pet Business pitch demo (Naaz + Studio + WhatsApp)
1. Base: Naaz Kool's cool grain aesthetic + `GrainOverlay` + `MarqueeTicker`
2. Add Studio's AI (Genkit/Gemini) as pet recommendation chatbot
3. Add Studio's `WhatsAppFloating` for instant inquiry
4. Add Studio's full shadcn/ui component library for polish
5. Result: Premium pet business site with AI + WhatsApp — perfect client pitch demo

### Example F: Freelancer portfolio with CMS (MorningStar standalone)
1. Take MorningStar Web as-is — already a complete agency CMS
2. Replace 3D scenes to match client's brand vibe
3. Add Studio's `WhatsAppFloating` for lead capture
4. Deploy to Vercel + connect Supabase
5. Result: Full portfolio + blog + project showcase + admin — hand over to client
